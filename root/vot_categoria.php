...
<form method="POST">
    ...
    if ($voted !=0 ){
      ...
      $tp_cat_cat = mysql_query("SELECT info,count(info) FROM votos WHERE id_planta='$id_plant' AND info != '' AND tipo = 'Categoria' AND $id_usuario = id_user"); 
      $opcao = mysql_result($tp_cat_cat, 0) ;     
         if ($vNome != $opcao){
           echo '
...
if (!isset($_COOKIE['login'])){
  ...
  <input type="submit" name= "enviar_no" value="Enviar">';
    ...
    echo '
  <input type="submit" name= "editar_c" value="Editar">';
}else{
    echo '
  <input type="submit" name= "enviar_c" value="Enviar">';
}
}
